"""
аписать функцию email_parse(<email_address>), которая при помощи регулярного
...
def email_parse(email_address):
    matching_groups = re.match(r'(?P<username>[\w.+-]+)@(?P<domain>[\w.+-]+\.[a-z]+)', email_address)
    try:
        ...
        result = matching_groups.groupdict()
        print(result)


email_parse('romka_ne@email.ru')
